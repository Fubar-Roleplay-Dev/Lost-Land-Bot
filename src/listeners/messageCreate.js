...
module.exports = async (client, msg) => {
  ...
  if (ticket) {
    ...
    if (ticketExpireId && author?.id === ticket.userId) {
      ...
      msg.channel.send('Automatic ticket expiry was cancelled by ticket user\'s message')
        .catch(() => { /* Void */ });
    ...
    // Track ticket admin activity
    if (!ticket.activeStaffIds.includes(member.id)) {
      ticket.activeStaffIds.push(member.id);
      await ticket.save();
    }
  }
