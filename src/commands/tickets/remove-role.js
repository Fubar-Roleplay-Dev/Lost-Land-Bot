...
module.exports = new ChatInputCommand({
  global: true,
  data: {
    description: 'Remove a role from this ticket',
    options: [
      {
        name: 'role',
        description: 'The role to remove from this ticket',
  ...
  run: async (client, interaction) => {
    ...
    try {
      ...
      interaction.editReply({
        content: `${ emojis.error } ${ member }, couldn't remove role ${ role } from ticket: ${ err.message }`,
    ...
    // Member feedback
    interaction.editReply({
      content: `${ emojis.success } ${ member }, I have remove ${ role } from this ticket`,
      disableMentions: true
    });

    // Ticket feedback
    channel
      .send(`${ emojis.success } <@${ ticket.userId }>, ${ role } was removed from this ticket`)
      .catch(() => { /* void */ });
